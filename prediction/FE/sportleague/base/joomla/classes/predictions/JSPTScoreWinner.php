...
class JSPTScoreWinner extends JSPTScore{
    public function __construct() {
        $db = JFactory::getDbo();
        $db->setQuery("SELECT * FROM #__bl_predtype WHERE identif='ScoreWinner'");
        $this->row = $db->loadObject();
    }
    public function getScore($match, $results) {
        if(($match->score1 < $match->score2 && $results['score1'] < $results['score2']) 
                || ($match->score1 > $match->score2 && $results['score1'] > $results['score2'])){
            return true;
        }else{
            return false;
        }
